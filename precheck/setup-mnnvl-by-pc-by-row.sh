...
HOSTNAME_IPS=$1
NUM_RACKS=$2
SERVERTOBT0_NDT=${SERVERTOBT0_NDT:-"/home/NetBench/fw_atl21_servertobt0.csv"}
MIN_NODES_PER_RACK=16
#BAD_RACK_ARR=("F02C04C003-EL424" "F02C04C003-EL421" "F02C04C003-EB425" "F02C04C003-EB423" "F02C04C003-EP430" "F02C04C003-EP429" "F02C04C003-EP428" "F02C04C003-EP427" "F02C04C003-EP425" "F02C04C003-EP422" "F02C04C003-EP421") 
...
all_ct_rack=$(
    ...
    sort |
    uniq -c |
    awk '{print $2, $3, $4, $1}' | # Rearrange to have pc, row, rack, count
    sort -k1,1 -k2,2 -k3,3
)
echo "$all_ct_rack" > all_ct_rack
all_ct_arr=$(echo "$all_ct_rack" | awk '{print $4}')
all_pc_arr=$(echo "$all_ct_rack" | awk '{print $1}')
all_row_arr=$(echo "$all_ct_rack" | awk '{print $2}')
mapfile -t all_rack_arr < <(echo "$all_ct_rack" | awk '{print $3}')
max_num_allracks="${#all_rack_arr[@]}"

ct_rack=$(echo "$all_ct_rack" | awk -v val=$MIN_NODES_PER_RACK '$4 >= val')
echo "$ct_rack" > ct_rack
ct_arr=$(echo "$ct_rack" | awk '{print $4}')
pc_arr=$(echo "$ct_rack" | awk '{print $1}')
row_arr=$(echo "$ct_rack" | awk '{print $2}')
mapfile -t rack_arr < <(echo "$ct_rack" | awk '{print $3}')
...
for rack in "${BAD_RACK_ARR[@]}"; do
    ...
    for ((i=0;i<$max_num_racks;i++)); do
        if [[ "${rack_arr[i]}" == "$rack" ]]; then
            unset 'rack_arr[i]'
            echo "Removing rack $rack from rack_arr"
        fi
    done
    for ((i=0;i<$max_num_allracks;i++)); do
        if [[ "${all_rack_arr[i]}" == "$rack" ]]; then
            unset 'all_rack_arr[i]'
            echo "Removing rack $rack from all_rack_arr"
        fi
    done
done
rack_arr=("${rack_arr[@]//}")
...
#fi
#racks_active=("${rack_arr[@]//}")
#all_racks_active=("${all_rack_arr[@]//}")
